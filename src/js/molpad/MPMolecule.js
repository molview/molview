 ...
 */
MPMolecule.prototype.rotateAtoms = function (
    center,
    point,
    atoms,
    currentAngle,
...
MPMolecule.prototype.mergeAtoms = function (src, dest, reverse) {
    ...
    for (
        ...
        var n = _dest.getNeighborBond(
            this.bonds[_src.bonds[j]].getOppositeAtom(src)
        );
        if (n === -1) {
            ...
            this.bonds[n].select(
                this.bonds[_src.bonds[j]].isSelected() ||
...
MPMolecule.prototype.collapseAtoms = function (atoms, reverse) {
    for (var i = 0; i < atoms.length; i++) {
        for (var j = 0; j < this.atoms.length; j++) {
            ...
            var distance =
                (!this.atoms[atoms[i]].isVisible() && !this.atoms[j].isVisible()
                    ? 1
                    : 2) * this.mp.s.atom.selectionRadiusScaled;

            if (
                this.atoms[atoms[i]].center.inCircle(
                    this.atoms[j].center,
                    distance
                )
            ) {
...
MPMolecule.prototype.removeAtom = function (index, rdh) {
    ...
    if (rdh) {
        for (var i = 0; i < this.atoms[index].bonds.length; i++) {
            var ai =
                this.bonds[this.atoms[index].bonds[i]].getOppositeAtom(index);
            if (
                this.atoms[ai].element == 'H' &&
                this.atoms[ai].bonds.length == 1
            )
                splice.push(ai);
        }
    }
...
// ----- SHP Multiple Paths -----
// Traversal policy for path finding
MPMolecule.prototype.canTraverse = function (
    fromIdx,
    bond,
    startIdx,
    goalIdx,
    ...
    opts = opts || {};
    var toIdx = bond.from === fromIdx ? bond.to : bond.from;

    // optional: skip hydrogens as intermediates (but allow if start/end is H)
    if (opts.excludeHydrogen) {
        var isMidpointH =
            this.atoms[toIdx].element === 'H' &&
            toIdx !== startIdx &&
            toIdx !== goalIdx;
...
// Shortest Path tool
MPMolecule.prototype.computeShortestPath = function (startAtom, endAtom, opts) {
    ...
    while (queue.length) {
        ...
        for (var bi = 0; bi < a.bonds.length; bi++) {
            ...
            // only traverse if allowed by policy
            if (!this.canTraverse(ai, b, startAtom.index, endAtom.index, opts))
                continue;
...
// ----- SHP Multiple Paths -----
// Enumerate all shortest paths (equal number of edges) between two atoms — ES5-safe
MPMolecule.prototype.computeAllShortestPaths = function (
    startAtom,
    endAtom,
    opts
) {
    if (!startAtom || !endAtom || startAtom === endAtom) return [];
    opts = opts || {};

    var start = startAtom.index,
        goal = endAtom.index,
        n = this.atoms.length;
    var dist = [],
        parents = [];
    ...
    while (q.length) {
        ...
        for (var bi = 0; bi < ua.bonds.length; bi++) {
            var b = this.bonds[ua.bonds[bi]];
            if (!this.canTraverse(u, b, start, goal, opts)) continue;

            var v = b.from === u ? b.to : b.from;
            if (dist[v] === Infinity) {
                dist[v] = dist[u] + 1;
                parents[v].push({ u: u, bond: b.index });
                q.push(v);
    ...
    var paths = [],
        scope = this;
    function backtrack(node, atomIdxs, bondIdxs) {
        if (node === start) {
            ...
            paths.push({ atoms: atomsPath, bonds: bondsPath });
            return;
        }
        ...
        for (var j = 0; j < preds.length; j++) {
            ...
            backtrack(p.u, atomIdxs, bondIdxs);
            atomIdxs.pop();
            bondIdxs.pop();
        }
    }
