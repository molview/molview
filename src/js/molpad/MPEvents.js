    ...
    // Shortest Path tool
        ...
        return {
            onPointerDown: function (e, mp) {
                ...
                // Hit test (same mechanism MolPad uses elsewhere)
                mp.handleEvent(p, 'active', function (obj) {
                    // Only atoms (have element + center)
                    if (
                        obj &&
                ...
                if (!hit) {
                    // Clicked empty area or a bond → ignore
                    return;
                }
                ...
                if (
                    ...
                    // Deselect previously highlighted path
                    ...
                    for (var i = 0; i < pa.length; i++) {
                        var ai = pa[i];
                        if (mp.mol.atoms[ai]) {
                            if (typeof mp.mol.atoms[ai].select === 'function') {
                                mp.mol.atoms[ai].select(false);
                            } else if (
                                typeof mp.mol.atoms[ai].setDisplay ===
                                'function'
                            ) {
                                mp.mol.atoms[ai].setDisplay('normal');
                            }
                        }
                    }
                    for (var j = 0; j < pb.length; j++) {
                        var bi = pb[j];
                        if (mp.mol.bonds[bi]) {
                            if (typeof mp.mol.bonds[bi].select === 'function') {
                                mp.mol.bonds[bi].select(false);
                            } else if (
                                typeof mp.mol.bonds[bi].setDisplay ===
                                'function'
                            ) {
                                mp.mol.bonds[bi].setDisplay('normal');
                            }
                        }
                    }

                    // Clear stored path indices
                    Sketcher.metadata.shortestPath2D = null;

                    // Start NEW selection with this click
                    mp.sel.clear();
                ...
                if (mp.tool.selection.length === 0) {
                    mp.sel.clear();
                    mp.tool.selection.push(hit);
                    hit.select(true); // show first pick
                    mp.requestRedraw();
                    return;
                }
                ...
                if (mp.tool.selection.length === 1) {
                    if (mp.tool.selection[0] === hit) return; // same atom, ignore
